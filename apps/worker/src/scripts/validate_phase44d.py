"""Phase 4.4D smoke validation: Replay Validation for Archetype-Aware Composite.

Checks:
  1. source/replay lineage exists or seeded test lineage path works
  2. archetype replay validation snapshot persists
  3. family archetype replay stability rows persist
  4. archetype replay validation summary rows populate
  5. family archetype replay stability summary rows populate
  6. archetype replay stability aggregate row populates
  7. unchanged archetype inputs produce validated / match-positive results
  8. controlled archetype / state / sequence / context change produces explicit
     drift reason codes
...
async def setup(conn: asyncpg.Connection) -> tuple[str, str]:
    ...
    if row:
        ...
        await conn.execute(
            ...
            "VALUES ($1::uuid, $2::uuid, 'phase44d_validation', 'Phase 4.4D Validation')",
...
async def seed_validation_row(
    ...
    *,
    ...
    archetype_attribution_match: bool, archetype_composite_match: bool,
    ...
    await conn.execute(
        """
        INSERT INTO cross_asset_archetype_replay_validation_snapshots
        ...
        VALUES ($1::uuid, $2::uuid, $3::uuid, $4::uuid, $5::uuid,
                ...
                '{}'::jsonb, '{}'::jsonb,
                $23::jsonb, $24,
                '{"scoring_version":"4.4D.v1","numeric_tolerance":1e-9}'::jsonb)
        """,
...
async def seed_family_stability_rows(
    ...
    *,
    ...
    fam_rows = [
        ("rates", 0.2200, 0.2200, 0.02376, 0.02376),
        ("risk",  0.1590, 0.1590, 0.01638, 0.01638),
    ]
    for (fam, s_attr, r_attr, s_int, r_int) in fam_rows:
        await conn.execute(
            """
            INSERT INTO cross_asset_family_archetype_replay_stability_snapshots
            ...
            """,
            workspace_id, watchlist_id, source_run_id, replay_run_id,
            fam, source_state, replay_state, source_seq, replay_seq,
            source_archetype, replay_archetype,
...
async def check_2_6(
    ...
    row = await conn.fetchrow(
        "SELECT id FROM cross_asset_archetype_replay_validation_snapshots "
    ...
    print(f"CHECK 2 PASSED: archetype_replay_validation_rows>=1 id={str(row['id'])[:12]}…")

    rows = await conn.fetch(
        "SELECT dependency_family FROM cross_asset_family_archetype_replay_stability_snapshots "
        "WHERE source_run_id = $1::uuid AND replay_run_id = $2::uuid",
        source_run_id, replay_run_id,
    )
    assert rows, "CHECK 3 FAILED: no family archetype replay stability rows"
    print(f"CHECK 3 PASSED: family_archetype_replay_rows={len(rows)}")

    srow = await conn.fetchrow(
        "SELECT validation_state FROM cross_asset_archetype_replay_validation_summary "
        "WHERE source_run_id = $1::uuid AND replay_run_id = $2::uuid",
        source_run_id, replay_run_id,
    )
    ...
    frows = await conn.fetch(
        "SELECT dependency_family, archetype_adjusted_delta "
    ...
    agg = await conn.fetchrow(
        ...
        "       archetype_match_rate, archetype_composite_match_rate "
        "FROM cross_asset_archetype_replay_stability_aggregate WHERE workspace_id = $1::uuid",
        workspace_id,
    )
    ...
    print(
        ...
        f"archetype_match_rate={agg['archetype_match_rate']} "
        f"archetype_composite_match_rate={agg['archetype_composite_match_rate']}"
    )
...
async def check_7_stable_match(
    ...
    row = await conn.fetchrow(
        ...
        SELECT context_hash_match, regime_match, timing_class_match,
               ...
               archetype_match,
               archetype_attribution_match, archetype_composite_match,
               archetype_dominant_family_match, validation_state
    ...
    assert row["archetype_attribution_match"]
    assert row["archetype_composite_match"]
    assert row["archetype_dominant_family_match"]
    assert row["validation_state"] == "validated", (
        f"CHECK 7 FAILED: expected 'validated', got {row['validation_state']!r}"
    )
    print("CHECK 7 PASSED: stable_archetype_replay_match_checked=true validation_state=validated")
...
async def check_8_drift_codes(
    ...
    """Seed a drift-scenario validation: context + archetype + transition +
    sequence all differ, producing explicit drift codes."""
    drift_codes = [
        "context_hash_mismatch",
        "transition_state_mismatch",
        "sequence_class_mismatch",
    ...
    await seed_validation_row(
        ...
        transition_state_match=False, sequence_class_match=False,
        archetype_match=False,
        archetype_attribution_match=True, archetype_composite_match=False,
        archetype_dominant_family_match=False,
        drift_codes=drift_codes,
        validation_state="drift_detected",
        source_timing_class="lead", replay_timing_class="lead",
        source_transition_state="reinforcing", replay_transition_state="deteriorating",
        source_sequence_class="reinforcing_path", replay_sequence_class="deteriorating_path",
        source_archetype_key="reinforcing_continuation",
        replay_archetype_key="deteriorating_breakdown",
    )
    row = await conn.fetchrow(
        """
    ...
    assert "archetype_key_mismatch" in codes, (
        f"CHECK 8 FAILED: archetype_key_mismatch missing from {codes}"
    )
    assert "transition_state_mismatch" in codes, (
        f"CHECK 8 FAILED: transition_state_mismatch missing from {codes}"
    )
    ...
    assert row["source_dominant_archetype_key"] == "reinforcing_continuation"
    assert row["replay_dominant_archetype_key"] == "deteriorating_breakdown"
    assert row["source_dominant_transition_state"] == "reinforcing"
    assert row["replay_dominant_transition_state"] == "deteriorating"
    print(f"CHECK 8 PASSED: archetype_drift_reason_codes_checked=true codes={codes}")
...
async def main() -> None:
    ...
    try:
        ...
        await seed_validation_row(
            ...
            archetype_attribution_match=True, archetype_composite_match=True,
            archetype_dominant_family_match=True,
        ...
        await seed_family_stability_rows(
            ...
            workspace_id=workspace_id, watchlist_id=watchlist_id,
            source_run_id=source_run_id, replay_run_id=replay_run_id,
            source_state="reinforcing", replay_state="reinforcing",
            source_seq="reinforcing_path", replay_seq="reinforcing_path",
