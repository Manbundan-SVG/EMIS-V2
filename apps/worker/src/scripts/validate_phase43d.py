"""Phase 4.3D smoke validation: Replay Validation for Sequencing-Aware Composite.

Checks:
  1. source/replay lineage exists or seeded test lineage path works
  2. sequencing replay validation snapshot persists
  3. family transition replay stability rows persist
  4. sequencing replay validation summary rows populate
  5. family transition replay stability summary rows populate
  6. sequencing replay stability aggregate row populates
  7. unchanged sequencing inputs produce validated / match-positive results
  8. controlled transition-state / sequence-class / context change produces
...
async def setup(conn: asyncpg.Connection) -> tuple[str, str]:
    ...
    if row:
        ...
        await conn.execute(
            ...
            "VALUES ($1::uuid, $2::uuid, 'phase43d_validation', 'Phase 4.3D Validation')",
...
async def seed_validation_row(
    ...
    *,
    ...
    transition_attribution_match: bool, transition_composite_match: bool,
    ...
    await conn.execute(
        """
        INSERT INTO cross_asset_transition_replay_validation_snapshots
        ...
        VALUES ($1::uuid, $2::uuid, $3::uuid, $4::uuid, $5::uuid,
                ...
                '{}'::jsonb, '{}'::jsonb,
                $20::jsonb, $21,
                '{"scoring_version":"4.3D.v1","numeric_tolerance":1e-9}'::jsonb)
        """,
...
async def seed_family_stability_rows(
    ...
    *,
    ...
    fam_rows = [
        ("rates", 0.2310, 0.2310, 0.02426, 0.02426),
        ("risk",  0.1908, 0.1908, 0.02061, 0.02061),
    ]
    for (fam, s_attr, r_attr, s_int, r_int) in fam_rows:
        await conn.execute(
            """
            INSERT INTO cross_asset_family_transition_replay_stability_snapshots
...
async def check_2_6(
    ...
    row = await conn.fetchrow(
        "SELECT id FROM cross_asset_transition_replay_validation_snapshots "
    ...
    print(f"CHECK 2 PASSED: transition_replay_validation_rows>=1 id={str(row['id'])[:12]}…")

    rows = await conn.fetch(
        "SELECT dependency_family FROM cross_asset_family_transition_replay_stability_snapshots "
        "WHERE source_run_id = $1::uuid AND replay_run_id = $2::uuid",
        source_run_id, replay_run_id,
    )
    assert rows, "CHECK 3 FAILED: no family transition replay stability rows"
    print(f"CHECK 3 PASSED: family_transition_replay_rows={len(rows)}")

    srow = await conn.fetchrow(
        "SELECT validation_state FROM cross_asset_transition_replay_validation_summary "
        "WHERE source_run_id = $1::uuid AND replay_run_id = $2::uuid",
        source_run_id, replay_run_id,
    )
    ...
    frows = await conn.fetch(
        "SELECT dependency_family, transition_adjusted_delta "
    ...
    agg = await conn.fetchrow(
        ...
        "       transition_state_match_rate, transition_composite_match_rate "
        "FROM cross_asset_transition_replay_stability_aggregate WHERE workspace_id = $1::uuid",
        workspace_id,
    )
    ...
    print(
        ...
        f"transition_state_match_rate={agg['transition_state_match_rate']} "
        f"transition_composite_match_rate={agg['transition_composite_match_rate']}"
    )
...
async def check_7_stable_match(
    ...
    row = await conn.fetchrow(
        ...
        SELECT context_hash_match, regime_match, timing_class_match,
               transition_state_match, sequence_class_match,
               transition_attribution_match, transition_composite_match,
               transition_dominant_family_match, validation_state
    ...
    assert row["transition_attribution_match"]
    assert row["transition_composite_match"]
    assert row["transition_dominant_family_match"]
    assert row["validation_state"] == "validated", (
        f"CHECK 7 FAILED: expected 'validated', got {row['validation_state']!r}"
    )
    print("CHECK 7 PASSED: stable_transition_replay_match_checked=true validation_state=validated")
...
async def check_8_drift_codes(
    ...
    """Seed a second drift-scenario validation: context + transition_state +
    sequence_class all differ, producing explicit drift codes."""
    drift_codes = [
    ...
    await seed_validation_row(
        ...
        transition_attribution_match=True, transition_composite_match=False,
        transition_dominant_family_match=False,
    ...
    assert "sequence_class_mismatch" in codes, (
        f"CHECK 8 FAILED: sequence_class_mismatch missing from {codes}"
    )
    ...
    assert row["source_dominant_sequence_class"] == "reinforcing_path"
    assert row["replay_dominant_sequence_class"] == "deteriorating_path"
    print(f"CHECK 8 PASSED: transition_drift_reason_codes_checked=true codes={codes}")
...
async def main() -> None:
    ...
    try:
        ...
        await seed_validation_row(
            ...
            transition_attribution_match=True, transition_composite_match=True,
            transition_dominant_family_match=True,
