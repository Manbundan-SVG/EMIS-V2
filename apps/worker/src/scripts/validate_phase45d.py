"""Phase 4.5D smoke validation: Replay Validation for Cluster-Aware Composite.

Checks:
  1. source/replay lineage exists or seeded test lineage path works
  2. cluster replay validation snapshot persists
  3. family cluster replay stability rows persist
  4. cluster replay validation summary rows populate
  5. family cluster replay stability summary rows populate
  6. cluster replay stability aggregate row populates
  7. unchanged cluster inputs produce validated / match-positive results
  8. controlled cluster/state/drift/context change produces explicit drift codes
...
async def setup(conn: asyncpg.Connection) -> tuple[str, str]:
    ...
    if row:
        ...
        await conn.execute(
            ...
            "VALUES ($1::uuid, $2::uuid, 'phase45d_validation', 'Phase 4.5D Validation')",
...
async def seed_validation_row(
    ...
    *,
    ...
    cluster_state_match: bool, drift_score_match: bool,
    cluster_attribution_match: bool, cluster_composite_match: bool,
    ...
    source_drift_score: float | None, replay_drift_score: float | None,
) -> str:
    vid = str(uuid.uuid4())
    await conn.execute(
        """
        INSERT INTO cross_asset_cluster_replay_validation_snapshots
        ...
        VALUES ($1::uuid, $2::uuid, $3::uuid, $4::uuid, $5::uuid,
                ...
                '{}'::jsonb, '{}'::jsonb,
                $29::jsonb, $30,
                '{"scoring_version":"4.5D.v1","numeric_tolerance":1e-9}'::jsonb)
        """,
...
async def seed_family_stability_rows(
    ...
    *,
    ...
    source_archetype: str, replay_archetype: str,
    drift_codes: list[str],
    cluster_state_match: bool, archetype_match: bool,
) -> None:
    fam_rows = [
        ("rates", 0.2376, 0.2376, 0.02566, 0.02566),
        ("risk",  0.1670, 0.1670, 0.01720, 0.01720),
    ]
    for (fam, s_attr, r_attr, s_int, r_int) in fam_rows:
        await conn.execute(
            """
            INSERT INTO cross_asset_family_cluster_replay_stability_snapshots
            ...
            """,
            workspace_id, watchlist_id, source_run_id, replay_run_id,
            fam, source_state, replay_state, source_archetype, replay_archetype,
...
async def check_2_6(
    ...
    row = await conn.fetchrow(
        "SELECT id FROM cross_asset_cluster_replay_validation_snapshots "
    ...
    print(f"CHECK 2 PASSED: cluster_replay_validation_rows>=1 id={str(row['id'])[:12]}…")

    rows = await conn.fetch(
        "SELECT dependency_family FROM cross_asset_family_cluster_replay_stability_snapshots "
        "WHERE source_run_id = $1::uuid AND replay_run_id = $2::uuid",
        source_run_id, replay_run_id,
    )
    assert rows, "CHECK 3 FAILED: no family cluster replay stability rows"
    print(f"CHECK 3 PASSED: family_cluster_replay_rows={len(rows)}")

    srow = await conn.fetchrow(
        "SELECT validation_state FROM cross_asset_cluster_replay_validation_summary "
        "WHERE source_run_id = $1::uuid AND replay_run_id = $2::uuid",
        source_run_id, replay_run_id,
    )
    ...
    frows = await conn.fetch(
        "SELECT dependency_family, cluster_adjusted_delta "
    ...
    agg = await conn.fetchrow(
        ...
        "       cluster_state_match_rate, drift_score_match_rate, cluster_composite_match_rate "
        "FROM cross_asset_cluster_replay_stability_aggregate WHERE workspace_id = $1::uuid",
        workspace_id,
    )
    ...
    print(
        ...
        f"cluster_state_match_rate={agg['cluster_state_match_rate']} "
        f"drift_score_match_rate={agg['drift_score_match_rate']} "
        f"cluster_composite_match_rate={agg['cluster_composite_match_rate']}"
    )
...
async def check_7_stable_match(
    ...
    row = await conn.fetchrow(
        ...
        SELECT context_hash_match, regime_match, timing_class_match,
               ...
               cluster_state_match, drift_score_match,
               cluster_attribution_match, cluster_composite_match,
               cluster_dominant_family_match, validation_state
    ...
    assert row["drift_score_match"]
    assert row["cluster_attribution_match"]
    assert row["cluster_composite_match"]
    assert row["cluster_dominant_family_match"]
    assert row["validation_state"] == "validated", (
        f"CHECK 7 FAILED: expected 'validated', got {row['validation_state']!r}"
    )
    print("CHECK 7 PASSED: stable_cluster_replay_match_checked=true validation_state=validated")
...
async def check_8_drift_codes(
    ...
    """Seed a drift-scenario validation: context + cluster_state + drift_score
    + composite all differ, producing explicit drift codes."""
    drift_codes = [
    ...
    await seed_validation_row(
        ...
        cluster_state_match=False, drift_score_match=False,
        cluster_attribution_match=True, cluster_composite_match=False,
        cluster_dominant_family_match=False,
        ...
        source_cluster_state="stable", replay_cluster_state="deteriorating",
        source_drift_score=0.10, replay_drift_score=0.65,
    )
    row = await conn.fetchrow(
        """
    ...
    assert "cluster_state_mismatch" in codes, (
        f"CHECK 8 FAILED: cluster_state_mismatch missing from {codes}"
    )
    assert "drift_score_mismatch" in codes, (
        f"CHECK 8 FAILED: drift_score_mismatch missing from {codes}"
    )
    ...
    assert row["source_cluster_state"] == "stable"
    assert row["replay_cluster_state"] == "deteriorating"
    assert float(row["source_drift_score"]) == 0.10
    assert float(row["replay_drift_score"]) == 0.65
    print(f"CHECK 8 PASSED: cluster_drift_reason_codes_checked=true codes={codes}")
...
async def main() -> None:
    ...
    try:
        ...
        await seed_validation_row(
            ...
            transition_state_match=True, sequence_class_match=True,
            archetype_match=True,
            cluster_state_match=True, drift_score_match=True,
            cluster_attribution_match=True, cluster_composite_match=True,
            cluster_dominant_family_match=True,
            ...
            source_drift_score=0.10, replay_drift_score=0.10,
        )
        await seed_family_stability_rows(
            conn,
            workspace_id=workspace_id, watchlist_id=watchlist_id,
            source_run_id=source_run_id, replay_run_id=replay_run_id,
            source_state="stable", replay_state="stable",
            source_archetype="reinforcing_continuation",
            replay_archetype="reinforcing_continuation",
            drift_codes=[],
            cluster_state_match=True, archetype_match=True,
        )
