...
import { CrossAssetConflictCompositePanel } from "@/components/cross-asset-conflict-composite-panel";
import { CrossAssetConflictReplayValidationPanel } from "@/components/cross-asset-conflict-replay-validation-panel";
...
export default function OpsDashboardShell({ workspaceSlug = "demo" }: { workspaceSlug?: string }) {
  ...
  const [conflictCompositeSummary, setConflictCompositeSummary] = useState<CrossAssetConflictCompositeSummaryRow[]>([]);
  const [familyConflictCompositeSummary, setFamilyConflictCompositeSummary] = useState<CrossAssetFamilyConflictCompositeSummaryRow[]>([]);
  const [finalConflictIntegrationSummary, setFinalConflictIntegrationSummary] = useState<RunCrossAssetConflictIntegrationSummaryRow[]>([]);
  const [conflictReplayValidationSummary, setConflictReplayValidationSummary] = useState<CrossAssetConflictReplayValidationSummaryRow[]>([]);
  const [familyConflictReplayStabilitySummary, setFamilyConflictReplayStabilitySummary] = useState<CrossAssetFamilyConflictReplayStabilitySummaryRow[]>([]);
  const [conflictReplayStabilityAggregate, setConflictReplayStabilityAggregate] = useState<CrossAssetConflictReplayStabilityAggregateRow | null>(null);
  ...
  const loadCrossAssetConflictComposite = useCallback(async () => {
    const res = await fetch(`/api/metrics/cross-asset-conflict-composite?workspace=${workspaceSlug}`, { cache: "no-store" });
    const data = await res.json() as {
      ok: boolean;
    ...
    if (data.ok) {
      setConflictCompositeSummary(data.conflictCompositeSummary ?? []);
      setFamilyConflictCompositeSummary(data.familyConflictCompositeSummary ?? []);
      setFinalConflictIntegrationSummary(data.finalConflictIntegrationSummary ?? []);
    }
  }, [workspaceSlug]);

  const loadCrossAssetConflictReplayValidation = useCallback(async () => {
    const res = await fetch(`/api/metrics/cross-asset-conflict-replay-validation?workspace=${workspaceSlug}`, { cache: "no-store" });
    const data = await res.json() as {
      ok: boolean;
    ...
    if (data.ok) {
      setConflictReplayValidationSummary(data.conflictReplayValidationSummary ?? []);
      setFamilyConflictReplayStabilitySummary(data.familyConflictReplayStabilitySummary ?? []);
      setConflictReplayStabilityAggregate(data.conflictReplayStabilityAggregate ?? null);
    }
  }, [workspaceSlug]);
  ...
  return (
    <main className="container">
      ...
      <CrossAssetConflictCompositePanel
        conflictCompositeSummary={conflictCompositeSummary}
        familyConflictCompositeSummary={familyConflictCompositeSummary}
        finalConflictIntegrationSummary={finalConflictIntegrationSummary}
        loading={loading}
      />

      <CrossAssetConflictReplayValidationPanel
        conflictReplayValidationSummary={conflictReplayValidationSummary}
        familyConflictReplayStabilitySummary={familyConflictReplayStabilitySummary}
        conflictReplayStabilityAggregate={conflictReplayStabilityAggregate}
        loading={loading}
      />
