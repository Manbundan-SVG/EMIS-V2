...
import { getGovernanceRoutingPolicyReviewMetrics } from "@/lib/queries/metrics";

export async function GET(request: NextRequest) {
  try {
    const workspace = request.nextUrl.searchParams.get("workspace") ?? "demo";
    const data = await getGovernanceRoutingPolicyReviewMetrics(workspace);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!["approved", "rejected", "deferred"].includes(body.reviewStatus)) {
      throw new Error("reviewStatus must be approved, rejected, or deferred");
    }
    ...
    const reviewInsert = await supabase
      .from("governance_routing_policy_recommendation_reviews")
      .insert({
